aggBounds_cont <- function(dat_final, seed = NULL, Y, X, S, D, W, Pscore,
  ...
  for (k in 1:cv_fold){
    ...
    if (is.null(seed)){
      q1.forest.lower <- quantile_forest(as.matrix(dat.s1.1[dat.s1.1[, S] == 1, X]), dat.s1.1[dat.s1.1[, S] == 1, Y], quantiles = one_q_l_ordered, regression.splitting = regression.splitting)
      q1.forest.upper <- quantile_forest(as.matrix(dat.s1.1[dat.s1.1[, S] == 1, X]), dat.s1.1[dat.s1.1[, S] == 1, Y], quantiles = one_q_u_ordered, regression.splitting = regression.splitting)

      q0.forest.lower <- quantile_forest(as.matrix(dat.s1.0[dat.s1.0[, S] == 1, X]), dat.s1.0[dat.s1.0[, S] == 1, Y], quantiles = one_q_l_ordered, regression.splitting = regression.splitting)
      q0.forest.upper <- quantile_forest(as.matrix(dat.s1.0[dat.s1.0[, S] == 1, X]), dat.s1.0[dat.s1.0[, S] == 1, Y], quantiles = one_q_u_ordered, regression.splitting = regression.splitting)
    }else{
      q1.forest.lower <- quantile_forest(as.matrix(dat.s1.1[dat.s1.1[, S] == 1, X]), dat.s1.1[dat.s1.1[, S] == 1, Y], seed = seed, quantiles = one_q_l_ordered, regression.splitting = regression.splitting)
      q1.forest.upper <- quantile_forest(as.matrix(dat.s1.1[dat.s1.1[, S] == 1, X]), dat.s1.1[dat.s1.1[, S] == 1, Y], seed = seed, quantiles = one_q_u_ordered, regression.splitting = regression.splitting)

      q0.forest.lower <- quantile_forest(as.matrix(dat.s1.0[dat.s1.0[, S] == 1, X]), dat.s1.0[dat.s1.0[, S] == 1, Y], seed = seed, quantiles = one_q_l_ordered, regression.splitting = regression.splitting)
      q0.forest.upper <- quantile_forest(as.matrix(dat.s1.0[dat.s1.0[, S] == 1, X]), dat.s1.0[dat.s1.0[, S] == 1, Y], seed = seed, quantiles = one_q_u_ordered, regression.splitting = regression.splitting)
    }
...
condBounds_cont <- function(dat_final, seed=NULL, Y, X, S, D, W, Pscore,
                            X_moderator, regression.splitting = F,
                            cv_fold = cv_fold){
...
LBounds_cont <- function(dat_final, seed = seed, Y, X, S, D, W, Pscore,
                         cond.mono = FALSE, direction = NULL){
