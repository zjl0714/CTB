aggBounds_binary_oneshot <- function(dat_bo, seed = NULL, Y, X, S, D, W, Pscore,
  ...
  for (k in 1:cv_fold){
    ...
    if (is.null(seed)){
      xi_model_help <- grf::probability_forest(as.matrix(dat.s1.1[dat.s1.1[, S] == 1, X]), as.factor(dat.s1.1[dat.s1.1[, S] == 1, Y]))
      xi_model_hurt <- grf::probability_forest(as.matrix(dat.s1.0[dat.s1.0[, S] == 1, X]), as.factor(dat.s1.0[dat.s1.0[, S] == 1, Y]))
    }else{
      xi_model_help <- grf::probability_forest(as.matrix(dat.s1.1[dat.s1.1[, S] == 1, X]), as.factor(dat.s1.1[dat.s1.1[, S] == 1, Y]), seed = seed)
      xi_model_hurt <- grf::probability_forest(as.matrix(dat.s1.0[dat.s1.0[, S] == 1, X]), as.factor(dat.s1.0[dat.s1.0[, S] == 1, Y]), seed = seed)
    }

    xi_pred_help <- stats::predict(xi_model_help, newdata = (as.matrix(X_eval_k)))
    xi_pred_hurt <- stats::predict(xi_model_hurt, newdata = (as.matrix(X_eval_k)))
...
aggBounds_binary <- function(dat_final, seed = NULL, Y, X, S, D, W, Pscore,
  ...
  ests <- aggBounds_binary_oneshot(dat_bo = dat_final, seed = seed, Y = Y, X = X, S = S, D = D, W = W, Pscore = Pscore,
                                   splitting = FALSE, cv_fold = cv_fold, trim_l = trim_l, trim_u = trim_u)
  ...
  # }
  #
  # se_tau_l_est_avg <- sqrt(stats::var(ests_boot[, 1]))
  # se_tau_u_est_avg <- sqrt(stats::var(ests_boot[, 2]))
...
LBounds_binary <- function(dat_final, Y, X, S, D, W, Pscore,
                           cond.mono = FALSE, Nboots = 1000, seed = NULL){
  ...
  if (cond.mono){
    ...
    se_tau_l_lee_hurt <- sqrt(stats::var(ests_lee_boot_hurt[, 1]))
    se_tau_u_lee_hurt <- sqrt(stats::var(ests_lee_boot_hurt[, 2]))
    ...
    se_tau_l_lee_help <- sqrt(stats::var(ests_lee_boot_help[, 1]))
    se_tau_u_lee_help <- sqrt(stats::var(ests_lee_boot_help[, 2]))
    ...
    if (direction){
      ...
      se_tau_l_lee <- sqrt(stats::var(ests_lee_boot[, 1]))
      se_tau_u_lee <- sqrt(stats::var(ests_lee_boot[, 2]))
    }else{
      cat("S(0) > S(1)", "\n")
