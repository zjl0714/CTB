CTTB <- function(data, seed = NULL, Y, D, S, X = NULL, W = NULL, Pscore = NULL,
  ...
  if (bo == 1){
    if (LeeBounds){
      ...
      TB_dat$CI95_lower <- TB_dat$Estimate - stats::qnorm(1 - alpha/2) * TB_dat$SE
      TB_dat$CI95_upper <- TB_dat$Estimate + stats::qnorm(1 - alpha/2) * TB_dat$SE
      if (IM_cv){
    ...
    if (aggBounds){
      agg.result <- aggBounds_binary(dat_final = dat_final, seed = seed, Y = Y, X = X, S = S, D = D, W = W, Pscore = Pscore,
                                     splitting = splitting, cv_fold = cv_fold, trim_l = trim_l, trim_u = trim_u)
      if (IM_cv){
      ...
      CTTB_dat$CI95_lower <- CTTB_dat$Estimate - stats::qnorm(1 - alpha/2) * CTTB_dat$SE
      CTTB_dat$CI95_upper <- CTTB_dat$Estimate + stats::qnorm(1 - alpha/2) * CTTB_dat$SE
      if (IM_cv){
        CTTB_dat$CI95_lower_IM <- CTTB_dat$Estimate - IM_cv_agg * CTTB_dat$SE
        CTTB_dat$CI95_upper_IM <- CTTB_dat$Estimate + IM_cv_agg * CTTB_dat$SE
      }
    ...
    if (aggBounds){
      agg.result <- aggBounds_cont(dat_final = dat_final, seed = seed, Y = Y, X = X, S = S, D = D, W = W, Pscore = Pscore,
                                   splitting = splitting,
                                   cv_fold = cv_fold, trim_l = trim_l, trim_u = trim_u)
      if (IM_cv){
        IM_cv_agg <- IM_cv(agg.result[["tau_l_est_avg"]], agg.result[["tau_u_est_avg"]],
                           agg.result[["se_tau_l_est_avg"]], agg.result[["se_tau_u_est_avg"]],
                           alpha, sum(dat_final[, S]))
    ...
    if (cBounds){
      ...
      cond_dat$CI95_lower <- cond_dat$Estimate - stats::qnorm(1 - alpha/2) * cond_dat$SE
      cond_dat$CI95_upper <- cond_dat$Estimate + stats::qnorm(1 - alpha/2) * cond_dat$SE
      # if (IM_cv){
...
IM_cv <- function(tau_l, tau_u, se_tau_l, se_tau_u, alpha, N){
  cv.fun <- function(cv, tau_l, tau_u, se_tau_l, se_tau_u, alpha, N){
    stats::pnorm(cv + (tau_u - tau_l) / max(se_tau_l, se_tau_u)) - stats::pnorm(-cv) - (1 - alpha)
  }
  cv.IM <- try(stats::uniroot(cv.fun, interval=c(-5, 5), tau_l, tau_u, se_tau_l, se_tau_u, alpha, N), silent = TRUE)
  return(cv.IM$root)
}

#' @export
print.CTTB <- function(x, ...) {
  result <- x
...
#' @export
plot.CTTB <- function(x, y = NULL, type = "aggregated", moderator_plot = 1, ...) {
  result <- x
  if (type == "aggregated"){
    if (result[["parameters"]]["LeeBounds"]){
      p.data <- result[["TB"]]
      ...
      stop("Aggregated bounds are not estimated")
    }
    pd <- ggplot2::position_dodge(width=0.2)
    coefs_p <- ggplot2::ggplot(p.data, ggplot2::aes(Method, Estimate, color=Type)) +
      ggplot2::geom_point(ggplot2::aes(shape=Method), size=4, position=pd, show.legend = FALSE) +
      ggplot2::scale_color_manual(name="Estimand",values=c("grey", "black")) +
      ggplot2::scale_shape_manual(name="Method",values=c(19, 19)) +
      ggplot2::theme_bw() +
      ggplot2::geom_errorbar(ggplot2::aes(Method, ymin=CI95_lower, ymax=CI95_upper), width=0.2, position=pd, linetype = "dashed") +
      ggplot2::xlab('') + ggplot2::ylab('') +
      ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5), text = ggplot2::element_text(size=25), axis.text.x = ggplot2::element_text(angle = 0), legend.position = "top") +
      ggplot2::ylab("Estimates") + ggplot2::coord_flip()

    y_min <- min(p.data$CI95_upper, na.rm = TRUE)

    if (result[["parameters"]]["IM_cv"]){
      coefs_p <- coefs_p + ggplot2::geom_errorbar(ggplot2::aes(Method, ymin=CI95_lower_IM, ymax=CI95_upper_IM), width=0.2, position=pd) +
        ggplot2::annotate("segment", x = 2.2, xend = 2.2, y = y_min - 0.6, yend = y_min - 0.2, linetype = "dashed", size = 0.5) +
        ggplot2::annotate("text", x = 2.2, y = y_min, label = "CI for the Bounds", hjust = 0, size = 5) +
        ggplot2::annotate("segment", x = 2.5, xend = 2.5, y = y_min - 0.6, yend = y_min - 0.2, linetype = "solid", size = 0.5) +
        ggplot2::annotate("text", x = 2.5, y = y_min, label = "IM CI for the Estimate", hjust = 0, size = 5)
    }else{
      coefs_p <- coefs_p +
        ggplot2::annotate("segment", x = 2.2, xend = 2.2, y = y_min - 0.6, yend = y_min - 0.2, linetype = "dashed", size = 0.5) +
        ggplot2::annotate("text", x = 2.2, y = y_min, label = "CI for the Bounds", hjust = 0, size = 5)
    ...
    if (result[["parameters"]]["cBounds"]){
      ...
      moderator_name <- rlang::sym(paste0("X", moderator_plot))
      if (stats::var(p.data.m[, paste0("X", moderator_plot)]) == 0){
        stop("No variation in the moderator. Plots cannot be produced.")
      }
      coefs_p <- ggplot2::ggplot(p.data.m, ggplot2::aes(x = !!moderator_name, y = Estimate, group = Type)) +
        ggplot2::geom_line(ggplot2::aes(y = CI95_lower, linetype = Type, color = Type), size = 1) +
        ggplot2::geom_line(ggplot2::aes(y = CI95_upper, linetype = Type, color = Type), size = 1) +
        ggplot2::geom_point(ggplot2::aes(color = Type), size = 3) +
        ggplot2::labs(x = "Moderator", y = "Conditional Bounds", color = NULL, linetype = NULL) +
        ggplot2::scale_color_manual(values = c("black", "gray")) +
        ggplot2::scale_linetype_manual(values = c("dashed", "dashed")) +
