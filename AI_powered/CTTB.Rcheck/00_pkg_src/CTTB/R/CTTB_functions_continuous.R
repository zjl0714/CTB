...
aggBounds_cont <- function(dat_final, seed = NULL, Y, X, S, D, W, Pscore,
                           splitting = FALSE, cv_fold = 5, trim_l, trim_u){
  ...
  for (k in 1:cv_fold){
    ...
    if (is.null(Pscore)){
      ...
      if (is.null(seed)){
        ps <- grf::probability_forest(as.matrix(dat.s3[, X]), as.factor(dat.s3[, D]), sample.weights = dat.s3[, W])
      }else{
        ps <- grf::probability_forest(as.matrix(dat.s3[, X]), as.factor(dat.s3[, D]), sample.weights = dat.s3[, W], seed = seed)
      }
      Ps_pred <- stats::predict(ps, newdata = (as.matrix(X_eval_k)))
    ...
    if (is.null(seed)){
      pf0 <- grf::probability_forest(as.matrix(dat.s1.0[, X]), as.factor(dat.s1.0[, S]), sample.weights = dat.s1.0[, W])
      pf1 <- grf::probability_forest(as.matrix(dat.s1.1[, X]), as.factor(dat.s1.1[, S]), sample.weights = dat.s1.1[, W])
    }else{
      pf0 <- grf::probability_forest(as.matrix(dat.s1.0[, X]), as.factor(dat.s1.0[, S]), sample.weights = dat.s1.0[, W], seed = seed)
      pf1 <- grf::probability_forest(as.matrix(dat.s1.1[, X]), as.factor(dat.s1.1[, S]), sample.weights = dat.s1.1[, W], seed = seed)
    }

    pf0_pred <- stats::predict(pf0, newdata = (as.matrix(X_eval_k)))
    pf1_pred <- stats::predict(pf1, newdata = (as.matrix(X_eval_k)))
    ...
    if (is.null(seed)){
      q1.forest.lower <- grf::quantile_forest(as.matrix(dat.s1.1[dat.s1.1[, S] == 1, X]), dat.s1.1[dat.s1.1[, S] == 1, Y], quantiles = one_q_l_ordered, regression.splitting = splitting)
      q1.forest.upper <- grf::quantile_forest(as.matrix(dat.s1.1[dat.s1.1[, S] == 1, X]), dat.s1.1[dat.s1.1[, S] == 1, Y], quantiles = one_q_u_ordered, regression.splitting = splitting)

      q0.forest.lower <- grf::quantile_forest(as.matrix(dat.s1.0[dat.s1.0[, S] == 1, X]), dat.s1.0[dat.s1.0[, S] == 1, Y], quantiles = one_q_l_ordered, regression.splitting = splitting)
      q0.forest.upper <- grf::quantile_forest(as.matrix(dat.s1.0[dat.s1.0[, S] == 1, X]), dat.s1.0[dat.s1.0[, S] == 1, Y], quantiles = one_q_u_ordered, regression.splitting = splitting)
    }else{
      q1.forest.lower <- grf::quantile_forest(as.matrix(dat.s1.1[dat.s1.1[, S] == 1, X]), dat.s1.1[dat.s1.1[, S] == 1, Y], seed = seed, quantiles = one_q_l_ordered, regression.splitting = splitting)
      q1.forest.upper <- grf::quantile_forest(as.matrix(dat.s1.1[dat.s1.1[, S] == 1, X]), dat.s1.1[dat.s1.1[, S] == 1, Y], seed = seed, quantiles = one_q_u_ordered, regression.splitting = splitting)

      q0.forest.lower <- grf::quantile_forest(as.matrix(dat.s1.0[dat.s1.0[, S] == 1, X]), dat.s1.0[dat.s1.0[, S] == 1, Y], seed = seed, quantiles = one_q_l_ordered, regression.splitting = splitting)
      q0.forest.upper <- grf::quantile_forest(as.matrix(dat.s1.0[dat.s1.0[, S] == 1, X]), dat.s1.0[dat.s1.0[, S] == 1, Y], seed = seed, quantiles = one_q_u_ordered, regression.splitting = splitting)
    }

    q1.forest.lower.pred <- stats::predict(q1.forest.lower, newdata = (as.matrix(X_eval_k)), quantiles = one_q_l_ordered)
    q1.forest.upper.pred <- stats::predict(q1.forest.upper, newdata = (as.matrix(X_eval_k)), quantiles = one_q_u_ordered)

    q0.forest.lower.pred <- stats::predict(q0.forest.lower, newdata = (as.matrix(X_eval_k)), quantiles = one_q_l_ordered)
    q0.forest.upper.pred <- stats::predict(q0.forest.upper, newdata = (as.matrix(X_eval_k)), quantiles = one_q_u_ordered)
  ...
  if (is.null(Pscore)){
    ...
    if (1 %in% directions_k & mean(scores_dat[scores_dat[, "directions_k"] == 1, "cor1s_k"]) > 0 & mean(scores_dat[scores_dat[, "directions_k"] == 1, "cor0s_k"]) > 0 & mean(scores_dat[scores_dat[, "directions_k"] == 1, "qar_help_k"]) > 0){
      ...
      V_l_help[1, 1] <- stats::var(scores_l_help_k)
      V_l_help[1, 2] <- V_l_help[2, 1] <- stats::cov(scores_l_help_k, qar_help_k)
      V_l_help[2, 2] <- stats::var(qar_help_k)

      V_u_help <- matrix(NA, 2, 2)
      V_u_help[1, 1] <- stats::var(scores_u_help_k)
      V_u_help[1, 2] <- V_u_help[2, 1] <- stats::cov(scores_u_help_k, qar_help_k)
      V_u_help[2, 2] <- stats::var(qar_help_k)
    ...
    if (0 %in% directions_k & mean(scores_dat[scores_dat[, "directions_k"] == 0, "cor1s_k"]) > 0 & mean(scores_dat[scores_dat[, "directions_k"] == 0, "cor0s_k"]) > 0 & mean(scores_dat[scores_dat[, "directions_k"] == 0, "qar_hurt_k"]) > 0){
      ...
      V_l_hurt[1, 1] <- stats::var(scores_l_hurt_k)
      V_l_hurt[1, 2] <- V_l_hurt[2, 1] <- stats::cov(scores_l_hurt_k, qar_hurt_k)
      V_l_hurt[2, 2] <- stats::var(qar_hurt_k)

      V_u_hurt <- matrix(NA, 2, 2)
      V_u_hurt[1, 1] <- stats::var(scores_u_hurt_k)
      V_u_hurt[1, 2] <- V_u_hurt[2, 1] <- stats::cov(scores_u_hurt_k, qar_hurt_k)
      V_u_hurt[2, 2] <- stats::var(qar_hurt_k)
    ...
    V_l <- matrix(NA, 2, 2)
    V_l[1, 1] <- stats::var(scores_l_k)
    V_l[1, 2] <- V_l[2, 1] <- stats::cov(scores_l_k, qar_k)
    V_l[2, 2] <- stats::var(qar_k)

    V_u <- matrix(NA, 2, 2)
    V_u[1, 1] <- stats::var(scores_u_k)
    V_u[1, 2] <- V_u[2, 1] <- stats::cov(scores_u_k, qar_k)
    V_u[2, 2] <- stats::var(qar_k)
...
condBounds_cont <- function(dat_final, seed=NULL, Y, X, S, D, W, Pscore,
  ...
  for (k in 1:cv_fold){
    ...
    if (is.null(Pscore)){
      if (is.null(seed)){
        ps <- grf::probability_forest(as.matrix(dat.s1[, X]), as.factor(dat.s1[, D]), sample.weights = dat.s1[, W])
      }else{
        ps <- grf::probability_forest(as.matrix(dat.s1[, X]), as.factor(dat.s1[, D]), sample.weights = dat.s1[, W], seed = seed)
      }
      Ps_pred <- stats::predict(ps, newdata = (as.matrix(X_eval_k)))
      Ps_eval_k <- Ps_pred$predictions[, 2]
    }else{
    ...
    if (is.null(seed)){
      pf0 <- grf::probability_forest(as.matrix(dat.s1.0[, X]), as.factor(dat.s1.0[, S]))
      pf1 <- grf::probability_forest(as.matrix(dat.s1.1[, X]), as.factor(dat.s1.1[, S]))
    }else{
      pf0 <- grf::probability_forest(as.matrix(dat.s1.0[, X]), as.factor(dat.s1.0[, S]), seed = seed)
      pf1 <- grf::probability_forest(as.matrix(dat.s1.1[, X]), as.factor(dat.s1.1[, S]), seed = seed)
    }

    pf0_pred_m <- stats::predict(pf0, newdata = (as.matrix(X_moderator)), estimate.variance = 1)
    pf1_pred_m <- stats::predict(pf1, newdata = (as.matrix(X_moderator)), estimate.variance = 1)
    ...
    if (is.null(seed)){
      q1.forest.lower.m <- grf::quantile_forest(as.matrix(dat.s1.1[dat.s1.1[, S] == 1, X]), dat.s1.1[dat.s1.1[, S] == 1, Y], quantiles = one_q_l_ordered_m, regression.splitting = 0)
      q1.forest.upper.m <- grf::quantile_forest(as.matrix(dat.s1.1[dat.s1.1[, S] == 1, X]), dat.s1.1[dat.s1.1[, S] == 1, Y], quantiles = one_q_u_ordered_m, regression.splitting = 0)
    }else{
      q1.forest.lower.m <- grf::quantile_forest(as.matrix(dat.s1.1[dat.s1.1[, S] == 1, X]), dat.s1.1[dat.s1.1[, S] == 1, Y], seed = seed, quantiles = one_q_l_ordered_m, regression.splitting = 0)
      q1.forest.upper.m <- grf::quantile_forest(as.matrix(dat.s1.1[dat.s1.1[, S] == 1, X]), dat.s1.1[dat.s1.1[, S] == 1, Y], seed = seed, quantiles = one_q_u_ordered_m, regression.splitting = 0)
    }
    q1.forest.lower.pred.m <- stats::predict(q1.forest.lower.m, newdata = (as.matrix(X_moderator)), quantiles = one_q_l_ordered_m)
    q1.forest.upper.pred.m <- stats::predict(q1.forest.upper.m, newdata = (as.matrix(X_moderator)), quantiles = one_q_u_ordered_m)
    ...
    if (is.null(seed)){
      q0.forest.lower.m <- grf::quantile_forest(as.matrix(dat.s1.0[dat.s1.0[, S] == 1, X]), dat.s1.0[dat.s1.0[, S] == 1, Y], quantiles = one_q_l_ordered_m, regression.splitting = 0)
      q0.forest.upper.m <- grf::quantile_forest(as.matrix(dat.s1.0[dat.s1.0[, S] == 1, X]), dat.s1.0[dat.s1.0[, S] == 1, Y], quantiles = one_q_u_ordered_m, regression.splitting = 0)
    }else{
      q0.forest.lower.m <- grf::quantile_forest(as.matrix(dat.s1.0[dat.s1.0[, S] == 1, X]), dat.s1.0[dat.s1.0[, S] == 1, Y], seed = seed, quantiles = one_q_l_ordered_m, regression.splitting = 0)
      q0.forest.upper.m <- grf::quantile_forest(as.matrix(dat.s1.0[dat.s1.0[, S] == 1, X]), dat.s1.0[dat.s1.0[, S] == 1, Y], seed = seed, quantiles = one_q_u_ordered_m, regression.splitting = 0)
    }
    q0.forest.lower.pred.m <- stats::predict(q0.forest.lower.m, newdata = (as.matrix(X_moderator)), quantiles = one_q_l_ordered_m)
    q0.forest.upper.pred.m <- stats::predict(q0.forest.upper.m, newdata = (as.matrix(X_moderator)), quantiles = one_q_u_ordered_m)
    ...
    if (is.null(seed)){
      rf0_m <- grf::regression_forest(as.matrix(dat.s2[dat.s2[, D] == 0 & dat.s2[, S] == 1, X]), Y = dat.s2[dat.s2[, D] == 0 & dat.s2[, S] == 1, Y])
    }else{
      rf0_m <- grf::regression_forest(as.matrix(dat.s2[dat.s2[, D] == 0 & dat.s2[, S] == 1, X]), Y = dat.s2[dat.s2[, D] == 0 & dat.s2[, S] == 1, Y], seed = seed)
    }
    pred0_m <- stats::predict(rf0_m, newdata = as.matrix(X_moderator), estimate.variance = 1)

    theta0_m <- pred0_m[, 1]
    se_theta0_m <- sqrt(pred0_m[, 2])

    if (is.null(seed)){
      rf1_m <- grf::regression_forest(as.matrix(dat.s2[dat.s2[, D] == 1 & dat.s2[, S] == 1, X]), Y = dat.s2[dat.s2[, D] == 1 & dat.s2[, S] == 1, Y])
    }else{
      rf1_m <- grf::regression_forest(as.matrix(dat.s2[dat.s2[, D] == 1 & dat.s2[, S] == 1, X]), Y = dat.s2[dat.s2[, D] == 1 & dat.s2[, S] == 1, Y], seed = seed)
    }
    pred1_m <- stats::predict(rf1_m, newdata = as.matrix(X_moderator), estimate.variance = 1)

    theta1_m <- pred1_m[, 1]
    se_theta1_m <- sqrt(pred1_m[, 2])

    if (is.null(seed)){
      cf_m <- grf::causal_forest(X = as.matrix(dat.s2[dat.s2[, S] == 1, X]),
    ...
    pred_m <- stats::predict(cf_m, newdata = as.matrix(X_moderator), estimate.variance = 1)
    tau_m[, k] <- pred_m[, 1]
    se_tau_m[, k] <- pred_m[, 2]
    ...
    for (m in 1:nrow(X_moderator)){
      ...
      if (is.null(seed)){
        rf1_l_m <- grf::regression_forest(as.matrix(dat.s2[dat.s2[, D] == 1 & dat.s2[, S] == 1, X]), Y = Y1.trim.lower)
        rf1_u_m <- grf::regression_forest(as.matrix(dat.s2[dat.s2[, D] == 1 & dat.s2[, S] == 1, X]), Y = Y1.trim.upper)
      }else{
        rf1_l_m <- grf::regression_forest(as.matrix(dat.s2[dat.s2[, D] == 1 & dat.s2[, S] == 1, X]), Y = Y1.trim.lower, seed = seed)
        rf1_u_m <- grf::regression_forest(as.matrix(dat.s2[dat.s2[, D] == 1 & dat.s2[, S] == 1, X]), Y = Y1.trim.upper, seed = seed)
      }
      pred1_l_m <- stats::predict(rf1_l_m, newdata = as.matrix(X_moderator), estimate.variance = 1)
      pred1_u_m <- stats::predict(rf1_u_m, newdata = as.matrix(X_moderator), estimate.variance = 1)
      ...
      if (is.null(seed)){
        rf0_l_m <- grf::regression_forest(as.matrix(dat.s2[dat.s2[, D] == 0 & dat.s2[, S] == 1, X]), Y = Y0.trim.lower)
        rf0_u_m <- grf::regression_forest(as.matrix(dat.s2[dat.s2[, D] == 0 & dat.s2[, S] == 1, X]), Y = Y0.trim.upper)
      }else{
        rf0_l_m <- grf::regression_forest(as.matrix(dat.s2[dat.s2[, D] == 0 & dat.s2[, S] == 1, X]), Y = Y0.trim.lower, seed = seed)
        rf0_u_m <- grf::regression_forest(as.matrix(dat.s2[dat.s2[, D] == 0 & dat.s2[, S] == 1, X]), Y = Y0.trim.upper, seed = seed)
      }
      pred0_l_m <- stats::predict(rf0_l_m, newdata = as.matrix(X_moderator), estimate.variance = 1)
      pred0_u_m <- stats::predict(rf0_u_m, newdata = as.matrix(X_moderator), estimate.variance = 1)
...
LBounds_cont <- function(dat_final, Y, X, S, D, W, Pscore,
  ...
  if (cond.mono){
    if (is.null(seed)){
      pf0 <- grf::probability_forest(X = as.matrix(dat_final[dat_final[, D] == 0, X]), Y = as.factor(unlist(dat_final[dat_final[, D] == 0, S])), sample.weights = dat_final[dat_final[, D] == 0, W])
      pf1 <- grf::probability_forest(X = as.matrix(dat_final[dat_final[, D] == 1, X]), Y = as.factor(unlist(dat_final[dat_final[, D] == 1, S])), sample.weights = dat_final[dat_final[, D] == 1, W])
    }else{
      pf0 <- grf::probability_forest(X = as.matrix(dat_final[dat_final[, D] == 0, X]), Y = as.factor(unlist(dat_final[dat_final[, D] == 0, S])), sample.weights = dat_final[dat_final[, D] == 0, W], seed = seed)
      pf1 <- grf::probability_forest(X = as.matrix(dat_final[dat_final[, D] == 1, X]), Y = as.factor(unlist(dat_final[dat_final[, D] == 1, S])), sample.weights = dat_final[dat_final[, D] == 1, W], seed = seed)
    }

    pf0_pred <- stats::predict(pf0, newdata = (as.matrix(dat_final[, X])))
    pf1_pred <- stats::predict(pf1, newdata = (as.matrix(dat_final[, X])))
