---
title: 'Tutorial for CTTB: An R Package for Estimating Covariate-Tightened Trimming Bounds'
output:
...
The results are shown in the plot above. The identified set from the CTTB method is again narrower than that from the TB method. The lower end of the 95\% confidence interval for the LATE estimate is above zero, suggesting that the LATE for the always-responders is positive. In the plot below, we compare the estimated bounds with the OLS estimate. The OLS estimate falls outside the identified set produced by CTTB in this case.
...
p
```
Next, we estimate the conditional bounds using age as the moderator. The results imply that the effect may be larger among younger subjects.
...
result <- CTTB(data = dat, seed = seed, Y = Y, D = D, S = S, X = X, W = W, cv_fold = 5, LeeBounds = TRUE, trim_l = 0.05, trim_u = 0.95)
