...
```{r agg-plot-truth, message = FALSE, warning = FALSE}
p + geom_hline(aes(yintercept = ate), colour = "red", lty=2) +
  geom_hline(aes(yintercept = late_ar), colour = "red", lty=4)
```

There are several other options under the control of researchers. *cv\_fold* decides how many folds of data are generated in cross-fitting. *splitting* determines whether regression splitting is activated in *grf*. *trim_l* and *trim_u* let researchers trim propensity score or conditional trimming probability estimates that are either too small or too large. *alpha* is the significance level.     
```{r agg-options, message = FALSE, warning = FALSE}
result <- CTTB(data = dat, seed = NULL, Y = "Y", D = "D", S = "S", X = c(names(dat)[c(2:12)]), Pscore = "Ps", cv_fold = 5, aggBounds = TRUE, LeeBounds = TRUE, cBounds = FALSE, X_moderator = NULL, cond_mono = FALSE)
...
With *CTTB()*, researchers can invesigate how
...
result <- CTTB(data = dat, seed = seed, 
               Y = Y, D = D, S = S, X = X, W = NULL, Pscore = "Ps",
               splitting = FALSE, cv_fold = 5,
               aggBounds = 1,
...
result.lee <- CTTB(data = dat, seed = seed, 
                   Y = Y, D = D, S = S, X = X, W = NULL, 
                   Pscore = "Ps", splitting = FALSE, cv_fold = 5,
                   aggBounds = 0, cBounds = 0, X_moderator = NULL, 
                   cond_mono = 1)
